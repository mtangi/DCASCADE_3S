function [ ResInundNodes, W_reach , Energy_slope_reach,h_reach, avgow_v ] = dam_features_correction( DamDatabase_active , ReachData, Network , ResVolume_t , Slope_t , Node_el_t)
...
avgow_v=cell(length(DamDatabase_active),1); % average flow velocity at each innundated edge  [m/s]
...
for d=1:length([DamDatabase_active.Node_ID])
    ...
    % if the reservoir volume is too low, the algorithm may results in
    % errors, thus in this case put all values to 0;
    if ResVolume_t(d) < DamDatabase_active(d).WL_table(3,3)
        
        reservoirFromN(d,1)=DamDatabase_active(d).Node_ID;

        ResInundNodes{d,1} = [Network.Upstream.Node{reservoirFromN(d,1)}];

        W_reach{d} = 0;
        Energy_slope_reach{d} = Slope_t(ResInundNodes{d,1});
        h_reach{d} = 0;
        avgow_v{d} = Inf;
        ...
        ResInundNodes{d,1} = [ReachData(   and(Network.Upstream.Distance{reservoirFromN(d)}~=Inf,Network.Upstream.Distance{reservoirFromN(d)}>0)).reach_id];
        ...
        % that effect. 
         ...
         if length(Lres)>1 % check if there is more than one innundated node
        ...
        avgow_v{d} = DamDatabase_active(d).mean_inflow./(h_reach{d}.*W_reach{d});
        %avgow_v{d} = Q_t([ReachData(ResInundNodes{d}).reach_id])./(h_reach{d}.*W_reach{d});
        Energy_slope_reach{d} = ((avgow_v{d}).^2./(2*9.807));
    end
    
end

%%%  write reservoir hydraulics table
%ReservoirHydraulics = table(reservoirFromN,ResInundNodes,Energy_slope_reach,W_reach,h_reach, avgow_v,'RowNames',{DamDatabase_active.Name}'); 

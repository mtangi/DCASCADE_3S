...
%reach n From_node
path2out = Network.Downstream.Path{n,1}{1,outlet};
...
Lngt_pathout = repmat(Lngt(path2out),n_class,1);
...
if n == outlet
    ...
    sed_pos = time_left' .* v_sed_path( sub2ind(size(transit_time) , 1:n_class ,indx_pos ) )' + downdist_path(indx_pos);

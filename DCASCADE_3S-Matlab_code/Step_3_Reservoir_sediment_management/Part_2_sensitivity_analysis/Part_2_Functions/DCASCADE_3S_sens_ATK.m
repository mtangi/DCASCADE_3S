function [ JJ] = DCASCADE_3S_sens_ATK(sed_input_param, theta)
%% DCASCADE_3S_sens_ATK
%
%% load data
...
for t = 2:timescale-1
    ...
    for n = NH
        ...
        elseif reaches_dam(n)>0 && dam_flushing(t,reaches_dam(n),1) == -2 % if flushing is occurring in flooded reaches 
            tr_cap = flushing_tr_cap( Fi_r_reach', EnergySlope(n) , Q(t,n)  , n_Man(n) , psi_3S )' .* 24.*60.*60;
        else

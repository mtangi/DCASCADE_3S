function [ JJ] = DCASCADE_3S_sens_EHW(sed_input_param, theta)
%% DCASCADE_3S_sens_EHW
%
%% load data
...
[Qbi_inputFL] = initialize_sed_input(ReachData, Fi_yield, Q, dates_Q,psi_3S, 10);
...
for t = 2:timescale-1
    ...
    for n = NH
        ...
        elseif reaches_dam(n)>0 && dam_flushing(t,reaches_dam(n),1) == -2 % if flushing is occurring in flooded reaches 
            Wac_FL = 12.8.* Q(t,n).^0.5; %change width in channel using flushing formula
            
            h_FL = (Q(t,n).*n_Man(n)./(Wac_FL.*sqrt( EnergySlope(n) ))).^(3/5);
            v_FL = 1./n_Man(n).*h_FL.^(2/3).*sqrt( EnergySlope(n) );        
            
            tr_cap = Engelund_Hansen_tr_cap(Fi_r_reach' , EnergySlope(n) , Wac_FL , v_FL , h_FL ,psi_3S)' .* 24.*60.*60;
